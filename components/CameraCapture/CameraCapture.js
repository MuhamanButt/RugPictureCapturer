...
export default function CameraCapture({ rugId, onComplete, onBack }) {
  ...
  return (
    <div className="camera-capture-container">
      <div className="header">
        ...
        <Modal open={isCapturing} footer={false} onCancel={()=>setIsCapturing(false)}>  
      ...
      <Button className="done-button" onClick={handleUploadAll} disabled={isUploading} >
        <Check className="icon-button" /> Done ({images.length + capturedImages.length})
      </Button>
        </Modal>
        ...
        <Card className="card upload-card">
          <CardContent className="card-content upload-content">
            ...
            <input
              ...
              className="hidden"
              onChange={(e) =>
                e.target.files && handleUploadWrapper(e.target.files)
              }
            />
            <Button onClick={() => fileInputRef.current?.click()} disabled={isUploading} className="upload-button" >
              <Upload className="icon-button" /> Upload from Device
            </Button>
            <p className="or-text">or</p>
            <Button onClick={startCamera} variant="outline" disabled={isUploading} className="camera-button" >
              <Camera className="icon-button" /> Use Camera
            </Button>
            {error && (
            ...
            {capturedImages.length > 0 && (
              <>
                <h3 className="section-title">
                  Captured Images (Pending Upload)
                </h3>
                <div className="images-grid captured-images-grid">
                  {capturedImages.map((img, i) => (
                    <div key={img.fileName} className="image-wrapper group">
                      <img src={URL.createObjectURL(img.blob)} className="image-thumb" alt={`Captured Image ${i + 1}`} />
                      <Button size="sm" variant="destructive" className="remove-button" onClick={() => removeCapturedImage(i)} disabled={isUploading} >
                ...
                <div className="images-grid uploaded-images-grid">
                  {images.map((img, i) => (
                    <div key={img.publicId} className="image-wrapper group">
                      <img src={img.secureUrl} className="image-thumb" alt={`Uploaded Image ${i + 1}`} />
                      <Button size="sm" variant="destructive" className="remove-button" onClick={() => removeUploadedImage(i)} disabled={isUploading}  >
                        <X className="icon-button" />
                      </Button>
                    </div>
                  ))}
